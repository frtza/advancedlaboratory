...
$\input{build/ddtt.tex}$ nähern, dessen Plateau bei $\input{build/nn.tex}$ in etwa dem Sollwert
$\qty{20}{\per\second}$ entspricht. Bei der Einstellung der optimalen Verzögerungszeit ist es in diesem
Fall egal, ob die Mitte des Plateaus $\input{build/opt.tex}$ oder alternativ die Mitte der Halbwertsbreite
$\input{build/alt.tex}$ verwendet werden, da die Skala nur halbzahlige Schritte zulässt.
...
Die Langzeitmessung zur Bestimmung der Lebensdauer in Abbildung \ref{fig:lifetime} folgt mit dem
...
$\input{build/tau.tex}$ als mittlere Zerfallszeit, deren Fehlerbereich den Literaturwert
$\qty{2.197}{\micro\second}$ \cite{Tishchenko_2013} beinhaltet und die somit als Ergebnis akzeptiert wird.

Anhand der Ausgleichrechnung gilt für das gegebene Messintervall von $\input{build/tmess.tex}$ eine
Hintergrundrate $\input{build/m.tex}$ durch fälscherlicherweise als Stopppuls gewertetes Eintreten weiterer
Myonen. Diese ist ebenso mit dem Wert $\input{build/mm.tex}$ aus der Poissonstatistik vereinbar.

Zuletzt kann noch eine weitere Struktur in den Daten aus Tabelle \ref{tab:lifetime} vermutet werden. Dabei
